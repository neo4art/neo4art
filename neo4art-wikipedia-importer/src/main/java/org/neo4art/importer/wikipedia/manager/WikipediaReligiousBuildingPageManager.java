 ...
 * @since 25.02.2015
 */
public class WikipediaReligiousBuildingPageManager extends WikipediaAbstractElementManager implements WikipediaElementManager {

	@Override
	public long createNodes(WikipediaElement wikipediaElement) {
	  
	  if (wikipediaElement.getType() != WikipediaType.RELIGIOUS_BUILDING_PAGE) {
	    throw new IllegalArgumentException("This method work only with wikipedia religious building pages but you called it with " + wikipediaElement.getType());
	  }

	  WikipediaRepository wikipediaRepository = new WikipediaBatchInserterRepository();
	  
	  return wikipediaRepository.addReligiousBuildingPage((WikipediaReligiousBuildingPage) wikipediaElement);
	}
}
