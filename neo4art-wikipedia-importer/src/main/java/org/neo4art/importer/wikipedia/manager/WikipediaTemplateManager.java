 ...
 * @since 25.02.2015
 */
public class WikipediaTemplateManager extends WikipediaAbstractElementManager implements WikipediaElementManager {

	@Override
	public long createNodes(WikipediaElement wikipediaElement) {
	  
	  if (wikipediaElement.getType() != WikipediaType.TEMPLATE) {
	    throw new IllegalArgumentException("This method work only with wikipedia templates but you called it with " + wikipediaElement.getType());
	  }

	  WikipediaRepository wikipediaRepository = new WikipediaBatchInserterRepository();
	  
	  return wikipediaRepository.addTemplate((WikipediaTemplate) wikipediaElement);
	}
}
