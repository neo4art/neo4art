...
public class WikipediaReligiousBuildingJainTempleInfobox {
	...
	public static ReligiousBuilding parse(String text)
		...
		ReligiousBuilding jainTemple = new ReligiousBuilding();
		...
		for (String key : map.keySet()) {
			switch (key) {
			case NAME:
				jainTemple.setBuildingName(WikipediaInfoboxUtils.removeAllParenthesis(map.get(key)));
				break;
			 case STYLE:
					jainTemple.setType(map.get(key));
					break;
			case IMAGE:
				jainTemple.setImage(WikipediaInfoboxUtils.infoboxImageUrl(map.get(key)));
				break;
			case LATD:
				coordinate.setLatD(Double.parseDouble(map.get(key)));
				jainTemple.setCoordinates(coordinate);
				break;
			case LATM:
