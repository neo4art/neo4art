 ...
 * @since 25.02.2015
 */
public class WikipediaFileManager extends WikipediaAbstractElementManager implements WikipediaElementManager {

	@Override
	public long createNodes(WikipediaElement wikipediaElement) {
	  
	  if (wikipediaElement.getType() != WikipediaType.FILE) {
	    throw new IllegalArgumentException("This method work only with wikipedia files but you called it with " + wikipediaElement.getType());
	  }

	  WikipediaRepository wikipediaRepository = new WikipediaBatchInserterRepository();
	  
	  return wikipediaRepository.addFile((WikipediaFile) wikipediaElement);
	}
}
