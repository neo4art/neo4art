 ...
 * @since 25.02.2015
 */
public class WikipediaSettlementPageManager extends WikipediaAbstractElementManager implements WikipediaElementManager {

	@Override
	public long createNodes(WikipediaElement wikipediaElement) {
	  
	  if (wikipediaElement.getType() != WikipediaType.SETTLEMENT_PAGE) {
	    throw new IllegalArgumentException("This method work only with wikipedia settlement pages but you called it with " + wikipediaElement.getType());
	  }
	  
	  WikipediaRepository wikipediaRepository = new WikipediaBatchInserterRepository();
	  
	  return wikipediaRepository.addSettlementPage((WikipediaSettlementPage) wikipediaElement);
	}
}
