 ...
 * @since 25.02.2015
 */
public class WikipediaCountryPageManager extends WikipediaAbstractElementManager implements WikipediaElementManager {

	@Override
	public long createNodes(WikipediaElement wikipediaElement) {
	  
	  if (wikipediaElement.getType() != WikipediaType.COUNTRY_PAGE) {
	    throw new IllegalArgumentException("This method work only with wikipedia country pages but you called it with " + wikipediaElement.getType());
	  }

	  WikipediaRepository wikipediaRepository = new WikipediaBatchInserterRepository();
	  
	  return wikipediaRepository.addCountryPage((WikipediaCountryPage) wikipediaElement);
	}
}
