 ...
 * @since 25.02.2015
 */
public class WikipediaArtMovementPageManager extends WikipediaAbstractElementManager implements WikipediaElementManager {

	@Override
	public long createNodes(WikipediaElement wikipediaElement) {
	  
	  if (wikipediaElement.getType() != WikipediaType.ART_MOVEMENT_PAGE) {
	    throw new IllegalArgumentException("This method work only with wikipedia art movement pages but you called it with " + wikipediaElement.getType());
	  }
	  
	  WikipediaRepository wikipediaRepository = new WikipediaBatchInserterRepository();
	  
	  return wikipediaRepository.addArtMovementPage((WikipediaArtMovementPage) wikipediaElement);
	}
}
