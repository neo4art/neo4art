 ...
 * @since 25.02.2015
 */
public class WikipediaMonumentPageManager extends WikipediaAbstractElementManager implements WikipediaElementManager {

	@Override
	public long createNodes(WikipediaElement wikipediaElement) {
	  
	  if (wikipediaElement.getType() != WikipediaType.MONUMENT_PAGE) {
	    throw new IllegalArgumentException("This method work only with wikipedia monument pages but you called it with " + wikipediaElement.getType());
	  }

	  WikipediaRepository wikipediaRepository = new WikipediaBatchInserterRepository();
	  
	  return wikipediaRepository.addMonumentPage((WikipediaMonumentPage) wikipediaElement);
	}
}
